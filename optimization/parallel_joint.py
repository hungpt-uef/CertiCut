...
def parallel_layer_gates(circuit: QuantumCircuit) -> tuple[ParallelLayerGate, ...]:
    """Extract only legal two-qubit layers; same-layer gates are necessarily disjoint."""
...
def evaluate_parallel_joint_partition(
    circuit: QuantumCircuit, partition: Sequence[int]
) -> ParallelJointEvaluation:
    """Evaluate executable PJ-QPD policy: optimal joint QPD per layer, layers independent."""
...
def exact_balanced_partitions(num_qubits: int) -> tuple[tuple[int, ...], ...]:
    """Enumerate symmetry-reduced exact balanced K=2 partitions with qubit zero fixed to side zero."""
    if num_qubits % 2:
