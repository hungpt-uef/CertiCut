...
\CertiCut{} contributes three linked results: (i)~a representation-placement theory built on tie-safe regret, margin stability, and positive-scaling invariance; (ii)~resource-semantic bounds $\Gamma_{\mathrm{inc}}/\Gamma^*\le\exp(\UB-\LB)$; and (iii)~an exact reduction of capacitated $K$-way independent-QPD cutting to a log-domain weighted graph partitioning problem. From a discrete-mathematical and graph-theoretic standpoint, this reduction establishes a new class of capacitated weighted graph partitioning models where edge weights exhibit representation-dependent structure rather than being static input constants. These results belong together because the MIP instantiates a set-level decision problem, while global bounds put representation-specific decisions on a common resource scale without conflating a solver-selected tie with a true reversal. Weighted graph partitioning, branch and bound, capacities, cardinality identities, and metric inequalities are established machinery~\cite{deza1997,lawler1966,kaibel2008}; our claim is the representation-placement analysis and its resource semantics, not a new generic partitioning algorithm.
...
If $w^{(b)}=c\,w^{(a)}$ for all edges in the common edge universe with scalar multiplier $c>0$, then the hierarchical objective ranking of all feasible partitions across $\mathcal{F}$ is strictly preserved. Consequently, the optimal-partition sets coincide, $\mathcal{A}_a=\mathcal{A}_b$, and $R_{a\to b}=R_{b\to a}=1$.
\end{corollary}
\begin{proof}
...
The lexicographic tolerance $\tau_{\mathrm{opt}}$ is not an arbitrary soft margin. It represents a mandatory mathematical separation boundary designed to prevent solver floating-point feasibility and integrality noise from interfering with the structural identification of the optimal solution set. Writing $\varepsilon_{\mathrm{SCIP}}$ for SCIP's effective feasibility/integrality tolerance (in SCIP both the constraint-feasibility and integrality checks are governed by \texttt{numerics/feastol}), we require
\begin{equation}
...
\end{equation}
whenever the relevant optimality margins are finite. The left-hand separation is enforced directly by the pinned \texttt{numerics/feastol} value. The right-hand separation is not independently verified in exact arithmetic on the medium tier; sweeping the declared values $\tau_{\mathrm{opt}}\in\{10^{-8},10^{-9},10^{-10}\}$ explicitly checks the numerical robustness and stability of the lexicographic MIP formulation across orders of magnitude, but does not replace an exact real-arithmetic decision proof for medium-scale instances. For the representation-regret rerun, every Stage-1 and Stage-2 SCIP model explicitly pins \texttt{numerics/feastol}$=10^{-12}$, so the sweep $\tau_{\mathrm{opt}}\in\{10^{-8},10^{-9},10^{-10}\}$ satisfies the left-hand separation. Exhaustive enumeration is used for $n\le10$; the lexicographic MIP covers $n\in\{12,14,16\}$ under PySCIPOpt~6.2.1 and Qiskit~2.5.1 / Addon Cutting~0.10.0.
